...
# **Data Preparation**
## Data Loading
  The next code is supposed to load the last required package for preparing, modeling and validating:
...
```{r d_arr, eval=TRUE, message=FALSE}
...
```{r d_arr2, eval=TRUE, message=FALSE}
set.seed(2021, sample.kind = "Rounding")
...
tune <- r$tune(train_data, opts = list(dim      = c(10, 20, 30),
                                       ...
                                       niter    = 10))
```
  The later used tune parameters can be found running the code:
